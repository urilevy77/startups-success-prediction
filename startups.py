...
def preprocessing(df):
    ...
    print("• closed_at: Fill with 2014-01-01 (business assumption: ongoing startups)")
    print("  Reasoning: Startups without closed_at are likely still operating")
    print("• Milestone dates: Keep NaN (indicates no milestones achieved)")
    ...
    print("\n🗂️ IRRELEVANT COLUMN HANDLING:")
    print("• Unnamed: 6: High missing percentage - likely formatting artifact, safe to drop")
    print("• state_code.1: Low missing percentage - duplicate of state_code, will be dropped")

    # Strategy 3: Milestone age columns - Keep NaN as meaningful
    print("\n📊 MILESTONE AGE COLUMNS:")
    print("• age_first_milestone_year & age_last_milestone_year: Missing values")
    print("• Decision: Fill with 0 (meaningful - indicates no milestones)")
    print("• Reasoning: Missing milestone data represents business reality")
    ...
    print("• Created 'age' feature: (closed_at - founded_at) in years")
    ...
    print("• Age categories: young (0-3), growing (3-7), mature (7-15), veteran (15+)")

    # One-hot encode age categories
    age_dummies = pd.get_dummies(df['age_category'], prefix='age')
    df = pd.concat([df, age_dummies], axis=1)
    print("• One-hot encoded age categories")
    ...
    def analyze_target_balance(df, target_col='labels'):
        ...
        if imbalance_ratio <= 3:
            ...
            print(f"• Imbalance ratio ({imbalance_ratio:.2f}:1) is within acceptable threshold (<3:1)")
            print("• Both classes are adequately represented")
            print("• Natural class distribution preserves real-world patterns")
            print("• Avoids overfitting risks from synthetic sample generation")
            print("• No need for SMOTE, ADASYN, or class_weight adjustments")

            print(f"\n📊 Distribution Analysis:")
            print(f"• Majority class: {(max_class/len(df)*100):.1f}% of data")
            print(f"• Minority class: {(min_class/len(df)*100):.1f}% of data")
            print("• Both classes have sufficient samples for robust learning")

        else:
            print(f"⚠️ MODERATE IMBALANCE DETECTED")
            print(f"• Consider balancing techniques for ratio {imbalance_ratio:.2f}:1")
    ...
    if 'labels' in df.columns:
        ...
        target_balance = None
        print("⚠️ No 'labels' column found for target analysis")
    ...
    print(f"• Features: {df.shape[1] - (1 if 'labels' in df.columns else 0)}")
    print(f"• Samples: {df.shape[0]:,}")
    if target_balance:
        print(f"• Target balance: {target_balance:.2f}:1 (No balancing needed)")
...
def feature_engineering_selection(df, target='labels'):
    ...
    print("\n• has_any: Basic funding success indicator")
    ...
    print("\n• has_Investor: Professional backing indicator")
    ...
    print("\n• has_Seed: Early-stage potential indicator")
    ...
    df['invalid_startup'] = np.where((df['has_any'] == 0) & (df['has_VC'] == 0) &
                                    (df['has_angel'] == 0), 1, 0)
    print("\n• invalid_startup: High-risk startup indicator")
    print("  Business Rationale: No funding or investors indicates")
    print("  either very early stage or struggling startup")

    print(f"\n📈 FEATURE ENGINEERING SUMMARY:")
    print(f"• Created {5} business logic features")
    print("• All features based on startup funding lifecycle domain knowledge")
    print("• Features capture different stages of startup maturity and success")
    ...
    print(f"• Removed {len(existing_drop_cols)} columns: {existing_drop_cols}")
    print("• Rationale: Keep numerical features and engineered business logic")
    ...
    print("• Method 1: Tree-based importance (XGBoost) - Captures non-linear relationships")
    print("• Method 2: Cross-validated importance - Validates feature stability")
    print("• Threshold: 'median' - Balanced approach removing bottom 50% features")
    ...
    print(f"✅ XGBoost selected {len(selected_features_xgb)} features:")
    print(f"• {selected_features_xgb.tolist()}")
    ...
    print("🔄 Performing cross-validated feature importance...")
    ...
    print("📊 Cross-validated feature importance (Top 10):")
    ...
    print(f"\n✅ CV-based selected {len(top_cv_features)} features:")
    print(f"• {top_cv_features}")
    ...
    print("\n🎯 SELECTION STRATEGY JUSTIFICATION:")
    print("Using XGBoost-based selection as primary method because:")
    print("• Tree-based methods handle non-linear relationships in startup data")
    print("• XGBoost naturally handles mixed data types (binary, continuous)")
    print("• Median threshold provides balanced feature reduction")
    print("• Method aligns with ensemble modeling approach")
    ...
    print(f"• Reduction ratio: {(1 - len(selected_features_final)/X_train.shape[1]):.1%}")
    print(f"• Features in common across all methods: {len(overlap_all)}")

    print("\n🏆 DOMAIN EXPERTISE VALIDATION:")
    business_features = ['has_all_rounds', 'has_any', 'has_Investor', 'has_Seed', 'invalid_startup']
    selected_business = [f for f in business_features if f in selected_features_final]
    print(f"• Business logic features selected: {len(selected_business)}/{len(business_features)}")
    print(f"• Selected business features: {selected_business}")
    print("• Validation: Domain-engineered features prove their predictive value")

    print("\n" + "=" * 60)
    print("FEATURE ENGINEERING & SELECTION COMPLETE")
    print("=" * 60)
    print("✅ Ready for Model Building with optimized feature set")
    print("✅ Feature importance validated through multiple methods")
    print("✅ Business logic features confirmed as predictive")
    print("=" * 60)
...
def model_building(df, features, target, models_to_check=None):
    ...
    print("• Baseline 1: Logistic Regression (Linear model)")
    print("• Baseline 2: Decision Tree (Simple non-linear)")
    print("• Advanced 1: XGBoost (Gradient boosting)")
    print("• Advanced 2: CatBoost (Category-optimized boosting)")
    print("• Advanced 3: AdaBoost (Adaptive boosting)")
    ...
    if models_to_check is None:
        ...
        print(f"\n🔧 USING DEFAULT MODEL SET:")
        print(f"• {len(models)} default models loaded")
    else:
        ...
        print(f"\n🔧 USING USER-PROVIDED MODEL SET:")
        print(f"• {len(models)} custom models provided")
    ...
    print(f"\n🔧 HYPERPARAMETER INFO:")
    if models_to_check is None:
        print("• Default XGBoost parameters CONSISTENT with feature selection phase")
        print("• REMOVED class_weight corrections (dataset is balanced)")
        print("• All default models use same random_state=42 for reproducibility")
    else:
        print("• Using user-provided model configurations")
        print("• Ensure your models have appropriate hyperparameters for your dataset")
    ...
    for name, model in models.items():
        ...
        print(f"   ✅ ROC-AUC: {roc_auc:.4f} | Accuracy: {accuracy:.4f} | CV: {cv_scores.mean():.4f}±{cv_scores.std():.4f}")
    ...
    print("=" * 60)
    print(f"✅ Returning {len(trained_models)} trained models")
...
def custom_hyperparameter_optimizer(models_dictionary, X, y, evaluation_method='precision', business_constraints=None):
    ...
    print(f"• Primary optimization metric: {evaluation_method}")
    print(f"• Models to optimize: {len(models_dictionary)}")
    ...
    for model_class, params in PARAMETER_GRIDS.items():
        ...
        print(f"• {model_class}: {param_combinations} combinations")
    ...
    def custom_constraint_optimizer(model, param_grid, X, y, min_precision, min_recall, min_f1, scoring, cv=5):
        ...
        print(f"\n🔍 OPTIMIZING {get_model_class_name(model)}:")
        print(f"• Parameter combinations: {np.prod([len(v) for v in param_grid.values()])}")
        print(f"• Constraints: Precision≥{min_precision}, Recall≥{min_recall}, F1≥{min_f1}")
        ...
        for params in grid_search.cv_results_['params']:
            ...
            try:
                ...
                print(f"   ⚠️ Error evaluating params {params}: {e}")
                continue
        ...
        print(f"   ✅ Complete: {optimization_time:.2f}s | Valid combinations: {len(valid_results)}/{len(all_results)}")
        if valid_results:
            print(f"   📊 Best: Precision={best_result['precision']:.3f}, Recall={best_result['recall']:.3f}, F1={best_result['f1']:.3f}")
        else:
            print(f"   ⚠️ No combinations meet all constraints")
    ...
    for model_name, model in models_dictionary.items():
        ...
        if model_class_name in PARAMETER_GRIDS:
            ...
            print(f"⚠️ No parameter grid found for {model_class_name}. Using basic grid.")
            # Basic fallback grid
            param_grid = {'random_state': [42]} if hasattr(model, 'random_state') else {}

        if not param_grid or len(param_grid) == 0:
            print(f"⚠️ Empty parameter grid for {model_name}. Skipping optimization.")
            # Use original model
    ...
    for model_name in optimized_models.keys():
        if model_name in optimization_results:
            ...
            comparison_data.append({
                ...
                'Constraints Met': '✅' if result['constraints_met'] else '❌',
    ...
    if comparison_data:
        ...
        if valid_models:
            ...
            print(f"\n🏆 BEST MODEL MEETING CONSTRAINTS: {best_model_name}")
            print(f"• Precision: {valid_models[best_model_name]['best_result']['precision']:.4f}")
            print(f"• Recall: {valid_models[best_model_name]['best_result']['recall']:.4f}")
            print(f"• F1-Score: {valid_models[best_model_name]['best_result']['f1']:.4f}")
            print(f"• ROC-AUC: {valid_models[best_model_name]['best_result']['roc_auc']:.4f}")
        else:
            print("\n⚠️ NO MODELS MEET ALL BUSINESS CONSTRAINTS")
    ...
    print(f"• Models meeting constraints: {len([r for r in optimization_results.values() if r['constraints_met']])}")
...
class StartupMLPipeline:
    ...
    def run_complete_pipeline(self, raw_data_path=None, raw_df=None,
        ...
        try:
            ...
            print("✅ Feature engineering and selection completed")
        ...
        try:
            ...
            print(f"📊 Models evaluated: {len(baseline_models)}")
        ...
        if not skip_optimization:
            ...
            print("\n⏭ STAGE 4: HYPERPARAMETER OPTIMIZATION SKIPPED")
            self.pipeline_results['optimization_success'] = False
        ...
        print(f"📊 Data: {df_raw.shape[0]} samples → {len(selected_features)} features")

        if self.pipeline_results.get('model_building_success', False):
            print(f"🏆 Best baseline model: {best_baseline_name} ({evaluation_method}: {best_baseline_score:.4f})")

        if self.pipeline_results.get('optimization_success', False):
            print(f"⚡ Best optimized model: {best_optimized_name} ({evaluation_method}: {best_optimized_score:.4f})")
            improvement = best_optimized_score - best_baseline_score
            print(f"📈 Optimization improvement: {improvement:.4f} ({improvement/best_baseline_score*100:.1f}%)")
...
print("• StartupMLPipeline class - Complete pipeline management")
print("• run_startup_prediction_pipeline() - Convenience function")
print("• All individual functions: preprocessing, feature_engineering_selection,")
